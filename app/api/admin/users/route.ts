...
export async function GET(){const a=await requireAuth(); await assertAdmin(a.uid); const q=await getAdminDb().collection('users').limit(200).get(); return NextResponse.json({users:q.docs.map(d=>d.data())});}
export async function PATCH(req:Request){const a=await requireAuth(); await assertAdmin(a.uid); const b=await req.json(); const ref=getAdminDb().collection('users').doc(b.uid); if(b.type==='grant'){const exp=new Date(Date.now()+Number(b.days||30)*86400000).toISOString(); await ref.set({adminAccessOverride:{enabled:true,expiresAt:exp},updatedAt:new Date().toISOString()},{merge:true});}
